...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    for i in range(1, len(sentences)):
        ...
        if sim < threshold:
            ...
            groups[curr_group_idx].append(sentences[i])

    chunks = []
    for idx, g in enumerate(groups):
        chunk_text = " ".join(g).strip()
        if chunk_text:
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for p in paragraphs:
        if current_parent_len + len(p) > parent_size and current_parent_paras:
            pid = f"parent_{len(parents)}"
            p_text = "\n\n".join(current_parent_paras).strip()
    ...
    for parent in parents:
        ...
        for unit in raw_units:
            if current_child_len + len(unit) > child_size and current_child_units:
                c_text = " ".join(current_child_units).strip()
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    for section in sections:
        ...
        if re.match(r'^#{1,3}\s+.+$', sec_clean):
            if current_header or current_content:
                full_text = (current_header + "\n\n" + "\n\n".join(current_content)).strip()
                if full_text:
                    clean_sec_name = re.sub(r'^#{1,3}\s+', '', current_header).strip() if current_header else "General"
                    chunks.append(Chunk(
                        text=full_text,
                        metadata={**metadata, "section": clean_sec_name, "strategy": "structure", "chunk_index": len(chunks)}
                    ))
            current_header = sec_clean
            current_content = []
        else:
            current_content.append(sec_clean)
