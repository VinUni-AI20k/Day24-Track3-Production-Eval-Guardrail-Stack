...
def run_ragas_50q(answers: list[dict]) -> list[RagasResult]:
    ...
    for i, a in enumerate(answers):
        if i < len(per_q):
            pq = per_q[i]
            f = float(getattr(pq, "faithfulness", 0.0))
            ar = float(getattr(pq, "answer_relevancy", 0.0))
            cp = float(getattr(pq, "context_precision", 0.0))
            cr = float(getattr(pq, "context_recall", 0.0))
        else:
...
def bottom_10(results: list[RagasResult]) -> list[dict]:
    ...
    for i, r in enumerate(bottom):
        diag, fix = DIAGNOSTIC_TREE.get(r.worst_metric, ("Unknown issue", "Review pipeline components"))
        output.append({
            "rank": i + 1,
