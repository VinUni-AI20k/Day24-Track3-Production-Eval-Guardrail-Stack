...
def run_ragas_50q(answers: list[dict]) -> list[RagasResult]:
    ...
    for index, item in enumerate(answers):
        ...
        if missing:
            raise ValueError(f"Answer at index {index} is missing fields: {sorted(missing)}")
    ...
    return [
        ...
        for answer, score in zip(answers, per_question)
    ]
...
def cluster_analysis(results: list[RagasResult]) -> dict:
    ...
    if not results:
        return {
            ...
            "insight": "No evaluation results were available for failure analysis.",
        }
    ...
    dominant_distribution = min(
        ...
        key=lambda distribution: (
            ...
            / max(sum(result.distribution == distribution for result in results), 1)
        ),
    )
    dominant_metric = max(matrix, key=lambda metric: sum(matrix[metric].values()))
    insight = (
        f"Distribution '{dominant_distribution}' has the lowest average score. "
